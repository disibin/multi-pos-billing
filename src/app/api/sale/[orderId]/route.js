...
import { authenticateStaff, isManagerOrAdmin } from '@/lib/auth';
...
export async function GET(req, { params }) {
  try {
    ...
    if (!auth.success) {
      return Response.json({ error: auth.message || 'Unauthorized staff access' }, { status: 401 });
    }
...
export async function DELETE(req, { params }) {
  ...
  if (!auth.success) {
    return Response.json({ error: auth.message || 'Access denied: Admin or Manager role required' }, { status: 403 });
  }
